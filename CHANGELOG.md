...
- The read loop's answer to every byte it can see, enumerated and held by tests. Each of the thirty-two bytes below a space, and the delete byte above the printable range, types a line around it, so what it did shows in the entry that comes back or in the error that ended it; the same is done for the bytes that are not valid UTF-8, where the number of replacement characters is what tells an overlong encoding from a surrogate. Both rules are now in the package documentation as well: a control byte the key map binds to nothing is dropped, because putting a raw control byte into the line would be worse than losing it, and a byte that is not valid UTF-8 is typed as U+FFFD, which is visible where dropping it would be silent and leaves the entry valid UTF-8 where keeping the byte would not. The mock terminal the tests drive holds the script as bytes and decodes it the way the real terminal does, rather than as runes: holding runes decoded them in the test file, where a byte that is not valid UTF-8 became U+FFFD before the prompt had seen it, so a test of what the prompt does with such a byte was a test of that conversion. Nothing in the package changed; what changed is that it is written down and checked, since the two bugs found in this surface last week were both in paths no test drove.

## [0.2.0] - 2026-09-02

### Changed

- The documentation says what a trailing backslash does. A line ending in one is continued rather than submitted -- in either mode, with or without `WithMultiline`, and whatever the `WithIsComplete` predicate says -- and the backslash is taken out of the entry, since it said how to read the line rather than being part of it. `WithMultiline`'s godoc said "without it Enter always submits", which this is the exception to.
...
- An entry can end in a backslash, by typing two ([#167](https://github.com/nao1215/prompt/issues/167)). Enter asked whether the line's last non-whitespace character was a backslash, so two of them continued the line as one did and one of the two was deleted: there was no way to end an entry in a backslash and no way to say so, while the obvious thing to try -- doubling it, which is what a shell takes as an escaped backslash -- ate a character instead. Enter now counts them. An odd number ends in a continuation marker, which is removed as before; an even number is data and the line submits with every one of them.
...
- A prefix that is not valid UTF-8 is drawn as what it is measured as ([#154](https://github.com/nao1215/prompt/issues/154)). Everything the renderer measures is measured by decoding runes, which turns an invalid byte into U+FFFD and counts the one cell that glyph occupies, while what was written was the raw byte -- so a terminal that drew it as nothing, or as two cells, left the caret that far from the text, and the two drawing paths did not agree with each other either. `singleLine` now normalizes a string that is not valid UTF-8, keeping its fast path for one that is.
...
- `example/demo`, a toy SQL shell showing context-aware completion, syntax highlighting and multi-line statements in one program, and the animation in the README recorded from it (PR [#120](https://github.com/nao1215/prompt/pull/120)). The animation it replaced was a screen recording of a desktop -- window chrome, a recorder's banner and somebody's shell prompt included -- made three months and forty fixes earlier, and nothing in the repository said it had gone stale. It is now generated by [VHS](https://github.com/charmbracelet/vhs) from a tape beside the program it drives, `make demo` records it, and a test fails when the tape, the program or the animation changes without the other two, so the README cannot go on showing something the library no longer does.

### Fixed

- A save no longer deletes entries the history file held without keeping them anywhere (PR [#119](https://github.com/nao1215/prompt/pull/119), [07750be](https://github.com/nao1215/prompt/commit/07750be), [#116](https://github.com/nao1215/prompt/issues/116)). A save writes over the file, so every entry the file holds and the save does not write is destroyed, and only one of the two ways that happens was asked about. `MaxFileSize` cuts what is written, which is the overflow `rotateIfNeeded` was given; `MaxEntries` cuts what is held, and it does it at load, before any save is considered, so the entries it dropped were not in the list that overflow is computed from and could not make it true. A history file with more entries than `MaxEntries` was cut down to `MaxEntries` on the next save with no backup taken, however much room `MaxFileSize` had left. The default limits are 1000 entries and 1MB and an ordinary history of one-line commands reaches 1000 entries long before it reaches a megabyte, so the entry limit is the binding one in practice and it was the one with nothing behind it: starting an application built on `WithFileHistory(path, 100)` and quitting again without typing deleted 400 entries of a 500-entry file, and there was nothing on disk to recover them from. A save now counts what the file holds and rotates when it is about to write fewer entries than that, which is the one question that covers both limits. The once-per-manager latch [#75](https://github.com/nao1215/prompt/issues/75) added is unchanged, so rotation still happens when the file fills rather than on every save.
- Enter on a reverse search that matched nothing leaves the line alone (PR [#119](https://github.com/nao1215/prompt/pull/119), [07750be](https://github.com/nao1215/prompt/commit/07750be), [#118](https://github.com/nao1215/prompt/issues/118)). It returned the query, so Ctrl+R followed by text nothing matches put that text on the command line as though it had been typed there -- and the next Enter, which is the natural thing to press when the first one appeared to do nothing, ran it. The block names the entry Enter would take and with no matches it names none, so there was nothing to accept; an empty history reaches this with no unusual typing at all, because every query fails when there is nothing to match. Enter with no matches now returns nothing, which is what Escape and Ctrl+C already return and what the read loop already handles by leaving the line as the search found it.
- A completion candidate or a recalled history entry holding an escape sequence is drawn rather than obeyed (PR [#115](https://github.com/nao1215/prompt/pull/115), [1ffd3b2](https://github.com/nao1215/prompt/commit/1ffd3b2), [#112](https://github.com/nao1215/prompt/issues/112)). The completion menu and the reverse-search block have replaced every control character with a space before drawing it since [#60](https://github.com/nao1215/prompt/issues/60) and [#61](https://github.com/nao1215/prompt/issues/61); the line being edited did not, and the line is where a candidate and a history entry end up. A menu row reading `select red` was written to the terminal as `select` followed by a real `ESC [ 31 m` the moment it was accepted: the rest of the line turned red and stayed red, and the cursor was drawn four columns right of the last character, because the layout walk measures an escape sequence as no cells while the terminal draws four of them. Every answer the wrap arithmetic gives inherited that -- the column the cursor is moved to, the rows the next redraw erases, where the menu is placed under the input. Neither route is typing: a typed ESC starts a key sequence and a paste is sanitized as it is inserted, so only text an application supplies arrives unfiltered, and a completer built from a file offers whatever the file holds. The line is now flattened once, before anything is measured or written. The buffer is left alone, so `Run` returns the string the completer offered and the history keeps it.
- A prefix holding an escape sequence draws the cursor past the text, and one holding a newline no longer walks the prompt down the screen (PR [#115](https://github.com/nao1215/prompt/pull/115), [1ffd3b2](https://github.com/nao1215/prompt/commit/1ffd3b2), [#113](https://github.com/nao1215/prompt/issues/113)). The prefix reaches the same measurement by another route: the application builds it, and one that builds it out of data -- sqly builds it from the working directory's name, and a directory name may hold any byte except `/` -- does not choose what goes in it. An escape sequence cost the cursor the sequence's length in columns. A newline cost a row instead, and not one the block counted: `calculateRenderedLines` splits the input on `\n` and never the prefix, so a two-row block was recorded as one, the erase that starts the next render was a row short, and the block was redrawn below the one before it -- three keystrokes left three copies of the prefix's first line on the screen. The prefix and the continuation prefix are now flattened the way the menu already was. This makes an embedded escape sequence visibly not work rather than quietly break the cursor, which is the honest answer for a renderer that measures its own layout: the prefix is text, and the scheme colors it. `Config.Prefix`, `Config.ContinuationPrefix` and `SetPrefix` say so.
- A candidate or a history entry holding a C1 control is flattened along with the rest (PR [#115](https://github.com/nao1215/prompt/pull/115), [1ffd3b2](https://github.com/nao1215/prompt/commit/1ffd3b2), [#114](https://github.com/nao1215/prompt/issues/114)). `leavesRow` decides what `singleLine` replaces, and its comment says it reports whether a terminal would do something other than draw the rune on the row it is on, while what it tested was `r < 0x20 || r == 0x7f` -- the C0 controls and DEL. That is half of Unicode's `Cc` category. The other half, U+0080 to U+009F, is executed by a terminal reading UTF-8: U+0085 is NEL, which takes the cursor to the next row, and U+009B is CSI, which opens a control sequence. Both measure zero cells, so a menu row holding one was drawn taller than it was counted and the extra row survived the erase -- the defect #60 and #61 named, still open for the range they did not cover. The test is now `unicode.IsControl`, which is exactly `Cc`. It stops there: U+2028 and U+2029 are line separators to a text layout engine and ordinary zero-width runes to a terminal, so flattening them would put a space where the terminal spends nothing, which is the same disagreement in the other direction. The C0 range is the half a person can type; the C1 range arrives only in data, from a file a completer read or a history file another program wrote, which is why the hand-written test covered one and not the other.
- Ctrl+Right no longer stops between a letter and the accent on it (PR [#110](https://github.com/nao1215/prompt/pull/110), [e88af68](https://github.com/nao1215/prompt/commit/e88af68), [#111](https://github.com/nao1215/prompt/issues/111)). A combining mark is in Unicode's Mark category rather than Letter, so word navigation treated it as a separator: on `café` typed or pasted in decomposed form -- an `e` followed by U+0301, which is what macOS hands back for a filename -- Ctrl+Right stopped after the `e` and left the accent ahead of the cursor. That is a cursor inside a character: Backspace there took the accent off a letter the user meant to keep, and the renderer, which measures a mark as no cells at all, drew the cursor on the letter's own cell, so the screen and the buffer disagreed about where the cursor was. A mark now counts as part of a word, which cannot join two words that were separate because a mark only ever follows what it belongs to. This is the input [#56](https://github.com/nao1215/prompt/issues/56) was about in the form it did not cover: it fixed `naïve` written with one rune and left the decomposed spelling alone.
...
- `MaxBackups: 0` keeps no backups, instead of three (PR [#119](https://github.com/nao1215/prompt/pull/119), [07750be](https://github.com/nao1215/prompt/commit/07750be), [#117](https://github.com/nao1215/prompt/issues/117)). `New` normalized `MaxBackups <= 0` to 3, while `rotateHistoryFile` has a branch implementing zero and `newHistoryManager` deliberately let zero through -- so the branch could never run from the public API, and a test in this repository already pinned the behavior it implements. Zero is a setting rather than an omission for this field: what the history file holds is what the user typed, which is where a password given on a command line ends up, and that is why [#74](https://github.com/nao1215/prompt/issues/74) made the file readable by its owner alone. An application that says it does not want copies of that content left beside the file now gets none.

  Migration: a `HistoryConfig` literal that leaves `MaxBackups` out now keeps no backups. Set it to 3 to keep what you had. Passing no `HistoryConfig` at all is unchanged and still takes every default, and `WithFileHistory` still asks for three.
