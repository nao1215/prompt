...
- `WithHighlighter` colours the input as it is drawn. It is given the whole input and returns the runs to draw in a colour of their own, as rune offsets into that input; everything no run covers keeps the scheme's colour. The prompt has no opinion about what the runs mean, so a highlighter for SQL colours keywords and string literals and one for a shell colours the command and its flags. It decides colours and nothing else: the input is drawn exactly as it is, and the prompt measures its layout from that text rather than from what is written to the terminal, so a highlighter cannot move the cursor away from the character under it or wrap a line early. A run reaching outside the input, an inverted one, and two that overlap are drawn over rather than rejected, because getting a colour wrong must not cost the user the line they are typing.
