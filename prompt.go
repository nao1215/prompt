...
type Config struct {
	...
	// Highlighter colours runs of the input as it is drawn. See WithHighlighter.
...
// WithHighlighter sets the function that colours the input as it is drawn.
//
// It is given the whole input and returns the runs to draw in a colour other
// than the scheme's Input colour, as rune offsets into that input. Everything
// no run covers keeps the scheme's colour. It is called on every render, which
...
// out of any one language's business: a highlighter for SQL colours keywords
// and string literals, one for a shell colours the command and its flags.
//
// It decides colours and nothing else. The input is drawn exactly as it is
...
// all drawn over rather than rejected: a colour is a decoration, and getting
